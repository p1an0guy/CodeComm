...
// deadlineConn observes HTTP/2 frame envelopes only to enforce bounded
// framing progress. The HTTP/2 implementation remains the sole parser.
...
func (connection *deadlineConn) SetDeadline(deadline time.Time) error {
	if connection == nil || connection.Conn == nil {
...
func (connection *deadlineConn) applyReadDeadlineLocked(now time.Time) error {
...
func (state *deadlineState) observe(
	data []byte,
	now time.Time,
	headerTimeout time.Duration,
	frameNoProgress time.Duration,
...
func (state *deadlineState) finishFrame() {
	switch {
	...
	case state.headerBlock:
		// The HTTP/2 implementation rejects interleaving. Preserve the
		// original header deadline until it does.
	default:
		state.phaseDeadline = time.Time{}
	}
