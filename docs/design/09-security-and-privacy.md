...
|---|---|
| Member floods proposals by fanning out to every peer | Per-receiver limits are a first-line filter only: proposals may enter through any member, so N peers each admit a quota and forward. §11.2's `leader_ingress_rate_per_device` is the actual bound, keyed by active signed `origin.device_id` rather than forwarding peer and applied before `raft.Apply`; inactive historical origins share one aggregate bucket and cannot churn active-origin state. Refusal is a local transport outcome, never a committed result |
