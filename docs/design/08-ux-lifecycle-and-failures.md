...
| Version-halted voter(s) | Stop their Raft instances and consensus routes; show the required release and unavailable voters. A compatible configured majority continues; otherwise strong writes and credential authorization pause until enough voters upgrade. Never offer §3.1 solely for version halt (§§4.6, 5.5) |
...
| Invite TTL expiry, three failed proofs, or SAS mismatch | Invite is voided with no retry; a failed proof does not consume the invite but increments its durable failure counter; after consumption, only an exact retry on the same TLS channel can recover a lost response, while connection/process loss requires a fresh invite (§4.5) |
| Pairing finalization is durably rejected | Mark the attempt `revoked`, report that admission/rebootstrap did not complete, require a fresh invite, and continue later finalization work (§4.5) |
